...
# -*- coding: utf-8 -*-
"""
ETVP v13.5 SOLIDSTATE — ФИНАЛЬНАЯ ОПТИМИЗАЦИЯ УДЕРЖАНИЯ
+30°C, городской и природный шум + СИСТЕМА УДЕРЖАНИЯ C + ФАКТОР ОПЕРАТОРА

Улучшения v13.5 (по сравнению с v13.4):
- Добавлен фактор оператора (C_OP) в эволюцию констант
- C_OP влияет на: alpha, mass_ratio, G
- Добавлена статистика по C_OP
- Возможность задать C_OP вручную или через профиль

Ожидаемый результат:
C      ≈ 0.8700 ± 0.005
1/α    ≈ 137.036 ± 0.05 (при C_OP=0.5)
m_p/m_e ≈ 1836.15 ± 0.4 (при C_OP=0.5)
G      ≈ 6.6743e-11 ± 1e-14 (при C_OP=0.5)
"""
...
TARGET_C = 0.8695      # скорректированная цель
C_MAX = 0.985
C_MIN = 0.05
V_MAX = 2.0
AVG_WINDOW = 200       # увеличенное окно
MEAS_WINDOW = 3
G_CALIBRATION = 1.0006 # калибровка G

# --- ФАКТОР ОПЕРАТОРА ---
C_OP = 0.95           # когерентность оператора (0..1)
ALPHA_OP = 0.5        # коэффициент связи (0.1..2.0)
OP_ENABLED = True     # включить/выключить фактор
...
# =============================================================================
# 2. СИСТЕМА УДЕРЖАНИЯ v13.5
# =============================================================================

class CoherenceKeeperV5:
    """Финальный ПИД с усиленным демпфированием."""
    ...
    def __init__(self):
        ...
        self.kp = 0.05
        self.ki = 0.005     # уменьшен
        self.kd = 0.015     # увеличен
...
class ETVPAmbientCoreV135:
    def __init__(self, memory_depth=200):
        ...
        self.op_factor = 1.0 + self.alpha_op * self.C_OP
        ...
        self.history = {
            ...
            "op_factor": []  # история фактора оператора
        }
    ...
    def evolve(self, t):
        ...
        # 9. Константы (с калибровкой G и фактором оператора)
        delta_C = self.C - 0.87
        delta_S = self.S - 0.15
        ...
        self.alpha_inv = (137.036 *
                          (1 + 0.1 * delta_C * (1 - self.S) + 0.05 * noise_filtered) *
                          self.op_factor)

        self.mass_ratio = (1836.15 *
                           (1 + 0.05 * delta_C * (1 - self.S) + 0.02 * noise_filtered) *
                           self.op_factor)

        G_raw = (6.6743e-11 *
                 (1 - 0.2 * delta_C * self.S + 0.1 * noise_filtered) *
                 self.op_factor)
        ...
        self.history["op_factor"].append(self.op_factor)
...
def run_simulation_v135():
    print("=" * 80)
    print("🌀 ETVP v13.5 — ФИНАЛЬНАЯ ОПТИМИЗАЦИЯ УДЕРЖАНИЯ + ФАКТОР ОПЕРАТОРА")
    print(f"   Температура среды: {T_ambient} K (+30°C)")
    print("   ПИД: kp=0.05, ki=0.005, kd=0.015, анти-виндап ±5.0")
    print(f"   Цель C: {TARGET_C}, окно усреднения: {AVG_WINDOW}, калибровка G: {G_CALIBRATION}")
    print(f"   ФАКТОР ОПЕРАТОРА: C_OP={C_OP}, ALPHA_OP={ALPHA_OP}, OP_ENABLED={OP_ENABLED}")
    print("=" * 80)

    core = ETVPAmbientCoreV135(memory_depth=200)
    steps = 15000  # больше шагов для стабилизации
    dt = PHOTON_DT_S
    ...
    for i in range(steps):
        ...
        if i % 1500 == 0:
            print(f"Шаг {i:5d}: C={result['C']:.4f}, α⁻¹={result['alpha']:.3f}, "
                  ...
                  f"OP={result['op_factor']:.3f}")

    # Статистика (последние 2000 шагов)
    n = 2000
    ...
    op_mean = np.mean(core.history["op_factor"][-n:])
    ...
    print(f"G      = {G_mean:.4e} ± {G_std:.2e} (CODATA: 6.6743e-11)")
    print(f"OP     = {op_mean:.4f} (фактор оператора)")

    print("\n--- РЕЗУЛЬТАТЫ (УСРЕДНЁННЫЕ, окно {AVG_WINDOW}) ---")
    ...
    print("\n--- ОТКЛОНЕНИЯ ОТ CODATA ---")
    print(f"1/α    : {abs(alpha_avg_mean - 137.036) / 137.036 * 100:.4f}%")
    print(f"mₚ/mₑ  : {abs(mass_avg_mean - 1836.15) / 1836.15 * 100:.4f}%")
    print(f"G      : {abs(G_avg_mean - 6.6743e-11) / 6.6743e-11 * 100:.4f}%")
    ...
    plt.tight_layout()
    plt.savefig('ambient_simulation_v135_results.png', dpi=150)
    plt.show()
