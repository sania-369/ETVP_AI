...
# -*- coding: utf-8 -*-
"""
ETVP v13.8 SOLIDSTATE — ШУМ СРЕДЫ + ОПЕРАТОР СВЕРХУ

Логика v13.8:
- БАЗА: ambient6 (v13.4) — шум среды, ПИД, Z-принцип
- ОПЕРАТОР: C_оп добавлен как ВТОРОЙ КОНТУР
- C_total = смесь C_system и C_оп
- Шум среды НЕ убирается — оператор стабилизирует систему
- Константы выводятся из C_total

Ожидаемый результат:
- C_оп = 0.0  → чистая система (как v13.4)
- C_оп = 0.6  → обычный оператор
- C_оп = 0.95 → высококогерентный оператор
...
C_OP = 0.95
OP_ENABLED = True
OP_WEIGHT = 0.5   # вес влияния оператора на C_total (0..1)
...
class MovingAverage:
    ...
    def get(self):
        return {
            ...
            'C_total': np.mean(self.buffers['C_total']) if self.buffers['C_total'] else 0
        }
...
class ETVPAmbientCoreV138:
    def __init__(self, memory_depth=200):
        ...
        self.C_total = TARGET_C     # итоговая когерентность (система + оператор)
    ...
    def evolve(self, t):
        """Один марковский такт: система + оператор."""
        self.step += 1

        # ====== КОНТУР СИСТЕМЫ (как v13.4) ======
        ...
        # ====== КОНТУР ОПЕРАТОРА (новое) ======
        # Оператор стабилизирует C_total, смешивая C_system и C_OP
        # C_total = C_system · (1 - w·C_OP) + C_OP · (w·C_OP)
        # где w = OP_WEIGHT
        w = OP_WEIGHT * self.C_OP
        self.C_total = self.C * (1.0 - w) + self.C_OP * w
        self.C_total = self.keeper.z_principle(self.C_total)

        # ====== КОНСТАНТЫ (из C_total) ======
        delta_C = self.C_total - 0.87
        delta_S = self.S - 0.15
        ...
        # ====== УСРЕДНЕНИЕ ======
        self.avg.update(self.alpha_inv, self.mass_ratio, self.G, self.C, self.C_total)
        ...
        self.history["C_total"].append(self.C_total)
        ...
        self.history["C_total_avg"].append(avg_vals['C_total'])

        return {"C": self.C, "C_total": self.C_total, "S": self.S,
...
def run_simulation_v138():
    print("=" * 80)
    print("🌀 ETVP v13.8 — ШУМ СРЕДЫ + ОПЕРАТОР СВЕРХУ")
    print(f"   Температура среды: {T_ambient} K (+30°C)")
    print("   ПИД: kp=0.05, ki=0.005, kd=0.015")
    print(f"   Цель C системы: {TARGET_C}")
    print(f"   C_OP = {C_OP}, OP_WEIGHT = {OP_WEIGHT}")
    print(f"   C_total = C_system·(1 - w·C_OP) + C_OP·(w·C_OP)")
    print("=" * 80)

    core = ETVPAmbientCoreV138(memory_depth=200)
    steps = 15000
    dt = PHOTON_DT_S
    ...
    for i in range(steps):
        ...
        if i % 1500 == 0:
            print(f"Шаг {i:5d}: C={result['C']:.4f}, C_total={result['C_total']:.4f}, "
    ...
    C_total_mean = np.mean(core.history["C_total"][-n:])
    C_total_std = np.std(core.history["C_total"][-n:])
    ...
    print(f"C_total   = {C_total_mean:.4f} ± {C_total_std:.4f}  (C_OP={C_OP})")
    print(f"Снижение σ: {C_std / C_total_std:.2f} раз" if C_total_std > 0 else "n/a")

    print("\n--- КОНСТАНТЫ (С ОПЕРАТОРОМ) ---")
    ...
    axes[0, 0].plot(core.history["C_total"], color='purple', alpha=0.7, label='C_total')
    axes[0, 0].axhline(TARGET_C, color='orange', linestyle='--', label='target')
    axes[0, 0].axhline(C_OP, color='red', linestyle=':', label=f'C_OP={C_OP}')
    axes[0, 0].set_title('Когерентность: система + оператор')
    axes[0, 0].legend()
    ...
    axes[1, 1].hist(core.history["C_total"][-n:], bins=50, alpha=0.5, color='purple', label='C_total')
    ...
    plt.tight_layout()
    plt.savefig('ambient_simulation_v138_results.png', dpi=150)
    plt.show()
