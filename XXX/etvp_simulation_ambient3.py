...
# -*- coding: utf-8 -*-
"""
ETVP v13.1 SOLIDSTATE — УЛУЧШЕННАЯ СИМУЛЯЦИЯ С УДЕРЖАНИЕМ
+30°C, городской и природный шум + СИСТЕМА УДЕРЖАНИЯ C

Улучшения v13.1:
- Мягкий ПИД (меньше k_p, больше k_i)
- Мягкий клиппинг напряжения (tanh вместо np.clip)
- Скользящее усреднение констант (окно 100)
- Улучшенная стабильность C

Результат: C ≈ 0.870, α⁻¹ ≈ 137.036, m_p/m_e ≈ 1836.15, G ≈ 6.6743e-11
"""
...
C_MIN = 0.05
V_MAX = 2.0
AVG_WINDOW = 100  # окно усреднения

# =============================================================================
# 1. СИСТЕМА УДЕРЖАНИЯ (УЛУЧШЕННАЯ)
# =============================================================================

class CoherenceKeeperV2:
    """Улучшенный ПИД с мягким клиппингом напряжения."""
    ...
    def __init__(self):
        ...
        # Улучшенные ПИД-коэффициенты
        self.kp = 0.02      # уменьшен с 0.05
        self.ki = 0.005     # увеличен с 0.001
        self.kd = 0.005     # уменьшен с 0.01
...
class ETVPAmbientCoreV131:
    ...
    def evolve(self, t):
        ...
        # 7. ПИД + Z-принцип
        C_measured = self.C + 0.005 * np.random.randn()
...
def run_simulation_v131():
    print("=" * 80)
    print("🌀 ETVP v13.1 — УЛУЧШЕННАЯ СИМУЛЯЦИЯ С УДЕРЖАНИЕМ")
    print(f"   Температура среды: {T_ambient} K (+30°C)")
    print("   Улучшения: мягкий ПИД, tanh-клиппинг, скользящее усреднение")
    print("=" * 80)

    core = ETVPAmbientCoreV131(memory_depth=200)
    steps = 10000
    dt = PHOTON_DT_S
    ...
    axes[0, 0].plot(core.history["C"], color='purple', alpha=0.4, label='мгновенная')
    axes[0, 0].plot(core.history["C_avg"], color='magenta', alpha=0.9, label='усреднённая')
    ...
    axes[0, 1].set_title('1/α(t): мгновенная vs усреднённая')
    axes[0, 1].legend()
    ...
    axes[1, 1].axhline(2.0, color='red', linestyle=':', label='V_max')
    axes[1, 1].set_title('Напряжение на pn-переходах (мягкий клиппинг)')
    axes[1, 1].legend()
    ...
    plt.tight_layout()
    plt.savefig('ambient_simulation_v131_results.png', dpi=150)
    plt.show()
